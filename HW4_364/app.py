...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://HarrisonDvoor@localhost/dvoorHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
class Gif(db.Model):
    ...
    def __repr__(self):
        return "{}/n Find the GIF Title and URL Here: {}".format(self.title, self.embedURL)
...
def get_gifs_from_giphy(search_string):
    ...
    param_dict = {'api_key':api_key, 'q':search_string, 'limit': 5}
    r = requests.get(baseurl, params=param_dict)
...
def get_or_create_search_term(db_session, term, gif_list = []):
    ...
    if search_term:
        ...
        for x in search:
            gif = get_or_create_gif(db_session, title = x['title'], url = x['embed_url'])
